...
def show_menu(s,price):
    if s == 1:
        ...
        if not datetime(2024,7,28)<=datetime.now()<=datetime(2024,8,11):
            i=1
            for k,v in menu_summer.items():
                print(i,end='.   ')
                print(k,v,'원')
                i+=1
            s= int(input('메뉴를 골라주세요:'))# 옵션 선택
...
if '케이크메뉴.csv' in os.listdir(folder):
    ...
    menu_cake={'Cake':5000}
if '여름특선메뉴.csv' in os.listdir(folder):
    with open('케이크메뉴.csv') as file:
        reader = csv.reader(file)
        for row in reader:
            # print(row)
    ...
    menu_summer={'Brunch':10000,'수박 주스':5000}
